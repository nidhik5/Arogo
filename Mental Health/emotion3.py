""" this code has been modified to include the decoder for the mental health analysis. 
input is webcam. output is graphs, objective 6 metrics, description and csv for mental health  metrics and emotion logs every 20 seconds
also downloads video of session and images of summary and metrics"""
...
# Set up video writer for output with description
...
def calculate_final_metrics(emotion_history):
    ...
    return {
        ...
        "avg_emotion_probabilities": avg_emotion_probabilities,  # Added for the decoder
        "mood_variance": mood_variance  # Added for the decoder
    }

# NEW FUNCTION: Generate mental health description based on metrics
def generate_mental_health_description(metrics):
    # Get top 3 emotions
    top_emotions = sorted(metrics["emotion_intensity"].items(), key=lambda x: x[1], reverse=True)[:3]
    top_emotions_str = ", ".join([f"{emotion} ({intensity:.1f}%)" for emotion, intensity in top_emotions])
    
    # Basic description template
    description = f"Mental Health Analysis Results:\n\n"
    
    # Add dominant emotions
    description += f"Dominant Emotions: {top_emotions_str}\n\n"
    
    # Add emotional state description based on polarity
    polarity = metrics["polarity_score"]
    if polarity > 0.5:
        emotional_state = "very positive emotional state"
    elif polarity > 0.2:
        emotional_state = "generally positive emotional state"
    elif polarity > -0.2:
        emotional_state = "neutral emotional state"
    elif polarity > -0.5:
        emotional_state = "somewhat negative emotional state"
    else:
        emotional_state = "very negative emotional state"
    
    description += f"Emotional State: The subject displays a {emotional_state} "
    description += f"with a polarity score of {polarity:.2f} (scale: -1 to +1).\n\n"
    
    # Add arousal description
    arousal = metrics["arousal_level"]
    description += f"Emotional Intensity: {arousal} arousal level. "
    
    if arousal == "High":
        description += "The subject shows intense emotional reactions.\n\n"
    elif arousal == "Medium":
        description += "The subject shows moderate emotional engagement.\n\n"
    else:
        description += "The subject shows subdued emotional responses.\n\n"
    
    # Add dominance description
    dominance = metrics["dominance_score"]
    if dominance > 0.3:
        dominance_desc = "High confidence and sense of control"
    elif dominance > -0.3:
        dominance_desc = "Balanced confidence level"
    else:
        dominance_desc = "Low confidence or feeling of submissiveness"
    
    description += f"Confidence Level: {dominance_desc} (score: {dominance:.2f}).\n\n"
    
    # Add stability description
    stability = metrics["mood_stability"]
    description += f"Emotional Stability: {stability}. "
    
    if stability == "Stable":
        description += "The subject maintains consistent emotional responses.\n\n"
    elif stability == "Unstable":
        description += "The subject shows some emotional variability.\n\n"
    else:
        description += "The subject displays significant emotional fluctuations.\n\n"
    
    # Add mental health indicator with explanation
    mental_health = metrics["mental_health"]
    description += f"Mental Health Indicator: {mental_health}. "
    
    if mental_health == "Normal":
        description += "The subject displays typical emotional patterns within expected ranges.\n\n"
    elif mental_health == "At Risk":
        description += "The subject shows some concerning emotional patterns that may benefit from attention.\n\n"
    else:
        description += "The subject displays significant emotional distress that may require support.\n\n"
    
    # Add disclaimer
    description += "Disclaimer: This analysis is based on computer vision and should not replace professional evaluation. "
    description += "Results are algorithmic interpretations and should be considered as preliminary insights only."
...
try:
    while cap.isOpened():
        ...
        if output_video is None:
            ...
            output_path = 'mental_health_analysis.mp4'
            output_video = cv2.VideoWriter(output_path, fourcc, fps, (frame_w, frame_h))
    ...
    if emotion_history:
        ...
        # Generate mental health description using the decoder
        mental_health_description = generate_mental_health_description(final_metrics)
        print("\n=== MENTAL HEALTH DESCRIPTION ===")
        print(mental_health_description)
        print("==============================")
        ...
        # Add the mental health description to the final frame of the video
        # Create a description image with the generated text
        description_img = np.zeros((720, 1280, 3), dtype=np.uint8)
        
        # Add title for description
        cv2.putText(description_img, "MENTAL HEALTH DESCRIPTION", 
                  (400, 50), cv2.FONT_HERSHEY_SIMPLEX, 1.2, (255, 255, 255), 2)
        
        # Add the description text with line wrapping
        description_lines = []
        words = mental_health_description.split()
        current_line = ""
        
        for word in words:
            test_line = current_line + " " + word if current_line else word
            # Check if we need to start a new line (max 60 chars per line)
            if len(test_line) > 60 or "\n" in word:
                if current_line:
        ...
        for i, line in enumerate(description_lines):
            y_pos = 120 + i * 30  # Adjust vertical spacing
            if y_pos < 700:  # Ensure we don't go off the bottom of the image
                cv2.putText(description_img, line, 
                          (100, y_pos), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 1)
        
        # Write the description frame to the video multiple times (5 seconds at 30fps = 150 frames)
        if output_video is not None:
            for _ in range(int(fps * 5)):  # 5 seconds
                output_video.write(description_img)
        ...
        cv2.imshow("Mental Health Description", description_img)
        cv2.waitKey(0)
